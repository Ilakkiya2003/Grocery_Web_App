...
export const Cart = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-100 to-gray py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-5xl font-bold text-cyan-800 mb-8 text-center">
          Your Cart
        </h1>
            ...
            {isEditingAddress ? (
              <div className="flex items-center">
                <Input
                  ...
                  className="flex-grow mr-2 border-cyan-300 focus:border-cyan-500 focus:ring-cyan-500"
                />
                <Button
                  onClick={() => handleAddressUpdate()}
                  className="bg-blue-500 hover:bg-cyan-600 text-white"
                >
                  Save
                </Button>
              </div>
        ...
        <Card className="mb-8">
          ...
          <CardContent>
            {cartProducts?.map((product) => (
              <div
                key={product.id}
                className="flex items-center py-4 border-b border-cyan-200 last:border-b-0"
              >
                <img
                  src={product.image}
                  alt={product.name}
                ...
                <div className="flex items-center">
                  <Button
                    ...
                    variant="outline"
                    size="icon"
                    className="h-8 w-8 rounded-full p-0 border-cyan-500 text-cyan-500 hover:bg-black-50"
                  >
                    <Minus className="h-4 w-4" />
                  </Button>
                  <span className="mx-2 font-semibold text-black-800">
                    {product.quantity}
                  </span>
                  <Button
                    onClick={() =>
          ...
          <CardFooter className="flex justify-between items-center">
            <div className="text-lg font-semibold text-cyan-800">Total:</div>
        ...
        <Button
          onClick={createOrder}
          className="w-full bg-cyan-500 hover:bg-cyan-600 text-white text-lg py-6 rounded-lg shadow-lg transition duration-300 ease-in-out transform hover:-translate-y-1"
        >
